...
export function SORManager() {
    ...
    const [items, setItems] = useState<SORItemFormData[]>([]);
    const [originalItems, setOriginalItems] = useState<Map<string, string>>(new Map());
    const [deleteItem, setDeleteItem] = useState<SORItemFormData | null>(null);
    ...
    const fetchItems = useCallback(async () => {
        try {
            ...
            const formData: SORItemFormData[] = data.map((item: SORItem) => ({
                id: item.id,
                name: item.name,
                isNew: false,
                isEditing: false,
            ...
            data.forEach((item: SORItem) => originalMap.set(item.id, item.name));
    ...
    const handleEditBlur = async (item: SORItemFormData) => {
        if (item.name !== originalItems.get(item.id)) {
            try {
                await axios.put(`/api/sor-items/${item.id}`, {
                    id: item.id,
                    name: item.name,
                });
                toast.success('SOR item updated successfully');
                ...
                toast.error(error.response?.data?.message || 'Failed to update SOR item');
                setItems(prev => prev.map(i =>
                    i.id === item.id
    ...
    const handleEditKeyDown = (e: React.KeyboardEvent, item: SORItemFormData) => {
        if (e.key === 'Enter') {
            handleEditBlur(item);
    ...
    const handleDeleteClick = (item: SORItemFormData) => {
        setDeleteItem(item);
    };
    ...
    const confirmDelete = async () => {
        ...
        try {
            ...
            toast.error(error.response?.data?.message || 'Failed to delete SOR item');
        } finally {
                        ...
                        <Table className='min-w-full'>
                            <TableHeader className="sticky top-0 [&_tr]:border-b">
                                <TableRow className="hover:bg-muted/50 data-[state=selected]:bg-muted border-b transition-colors">
                                    ...
                                    >
                                        SOR Name {sortField === 'item_name' && (sortOrder === 'asc' ? '↑' : '↓')}
                                    </TableHead>
                                                ...
                                                {item.isEditing || item.isNew ? (
                                                    <div className="space-y-1">
                                                        <Input
                                                            ...
                                                            placeholder="Enter SOR item name"
